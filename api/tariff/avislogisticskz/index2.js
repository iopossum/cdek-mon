  ...
  async.auto({
    getCities: function (callback) {
      ...
      async.retry(config.retryOpts, function (callback) {
        ...
        var $ = cheerio.load(b);
        var countryOpts = $($('[name="countryFrom"]')[0]).find('option');
        var cityOpts = $($('[name="cityFrom"]')[0]).find('option');
        var servicesOpts = $($('[name="tarif"]')[0]).find('option');
        var countriesObj = {};
        countryOpts.each(function (index, item) {
          if (index !== 0) {
            var id = $(item).attr('value');
            countriesObj[id] = {id: id, name: $(item).text().toUpperCase().trim(), cities: []};
          }
        });
        cityOpts.each(function (index, item) {
          var id = $(item).attr('data-country');
          if (countriesObj[id]) {
            countriesObj[id].cities.push({id: $(item).attr('value'), name: $(item).text().toUpperCase().trim(), countryId: id});
          }
        });
        var services = [];
        servicesOpts.each(function (index, item) {
          if (index !== 0) {
            services.push({id: $(item).attr('value'), name: $(item).text().toUpperCase().trim()});
          }
        });
        callback(null, {services: services, cities: _.values(countriesObj)});
      });
    },
    parseCities: ['getCities', function (results, callback) {
      var countryObj = _.indexBy(results.getCities.cities, 'name');
