        ...
        setTimeout(function () {
          ...
          async.series([
            ...
            function (callback) {
              ...
              getCity(city.from, city.countryFrom || city.countryTo, city.cookie, callback);
            },
            function (callback) {
              if (city.countryTo) {
                return callback(null, {isCountry: true, success: true, foundCities: [countryObj[city.countryTo.toUpperCase()]]});
              }
              getCity(city.to, city.countryFrom || city.countryTo, city.cookie, callback);
            }
          ...
          item.fromJson.foundCities.forEach(function (fromCity) {
            item.toJson.foundCities.forEach(function (toCity) {
              if (item.fromJson.isCountry || item.toJson.isCountry) {
                ...
                requests = requests.concat(commonHelper.getResponseArray(req.body.weights, item, delivery, "Международная доставка недоступна"));
              } else {
                tempRequests.push({
                  city: {
                    initialCityFrom: item.from,
